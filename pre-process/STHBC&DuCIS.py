...
def extract_UNI2h_features_from_spots(model, image_path, positions, crop_size=299, device=None):
    ...
    Logic is consistent with the HER2ST processing pipeline.
    """
    if device is None:
...
def read(ref_dir, tgt_dir, ref_name, tgt_name, n_genes=768, overlap=None, preprocess=False):
    ...
    ref_expr_raw = []  # Stores normalized reference adata
    tgt_img, tgt_img_resnet, tgt_pos, tgt_geneEmb = [], [], [], []
    tgt_expr_raw = None  # Stores normalized target adata
    ...
    # 1. Process Reference Datasets (Initial Normalization)
    print("Step 1: Processing reference datasets...")
    ref_adata_list = []  # Original adata (raw)
    ref_adata_processed = []  # Normalized + log1p adata
    ...
    for r in ref_name:
        ...
        # Initial normalization and log-transformation (performed only once here)
        adata_processed = adata.copy()
        sc.pp.normalize_total(adata_processed, target_sum=1e4)
        sc.pp.log1p(adata_processed)
        ref_adata_processed.append(adata_processed)

    # 2. Process Target Dataset (Initial Normalization)
    ...
    # Initial normalization and log-transformation (performed only once here)
    tgt_adata_processed = tgt_adata_initial.copy()
    sc.pp.normalize_total(tgt_adata_processed, target_sum=1e4)
    sc.pp.log1p(tgt_adata_processed)

    # 3. Select Top 80% HVGs for All Datasets
    print("\nStep 3: Selecting top 80% HVGs for all datasets...")
    ref_adata_hvg_list = []

    # Process Reference (using processed/normalized adata)
    for i, (r, adata) in enumerate(zip(ref_name, ref_adata_processed)):
    ...
    # Process Target (using processed/normalized adata)
    print(f"  - Target {tgt_name}: Selecting HVGs...")
    total_genes_tgt = tgt_adata_processed.shape[1]
    top_80_percent_tgt = int(total_genes_tgt * 0.8)
    sc.pp.highly_variable_genes(tgt_adata_processed, flavor="seurat_v3", n_top_genes=top_80_percent_tgt, subset=False)
    hvg_80pct_tgt = tgt_adata_processed.var[tgt_adata_processed.var['highly_variable']].index.tolist()
    tgt_adata_hvg = tgt_adata_processed[:, hvg_80pct_tgt]

    # 4. Find Global Gene Intersection
    print("\nStep 4: Finding global gene intersection (ref + tgt HVGs)...")
    overlap_genes = set(ref_adata_hvg_list[0].var_names)

    # Intersect with other reference datasets
    ...
    # 5. Filter Datasets using Intersection
    print("\nStep 5: Filtering all datasets with the global gene intersection...")
    # These datasets are already normalized and filtered by the intersection gene set
    ...
    # NOTE: Subsequent processing uses the already normalized `ref_adata_filtered`
    # and `tgt_adata_filtered`. No re-normalization is performed.
    ...
    for i, (r, adata) in enumerate(zip(ref_name, ref_adata_filtered)):
        ...
        ref_expr_raw.append(adata.copy())  # Store normalized adata
        ref.append(adata)
    ...
    print(f"  - Extracting UNI2h features...")
    # Note: crop_size=300 used for target dataset in this pipeline
    tgt_image_feats = extract_UNI2h_features_from_spots(
        model_UNI2h, tgt_image_path, tgt_position, crop_size=300, device=device
    )
    ...
    tgt_resnet_feats = extract_resnet_features_from_spots(
        tgt_image_path, tgt_position, crop_size=300, device=device
    )
    ...
    tgt_expr_raw = adata.copy()  # Store normalized adata
    ...
    ref_expr_filtered_1536 = [r.copy() for r in ref_expr_raw]  # Use normalized data
