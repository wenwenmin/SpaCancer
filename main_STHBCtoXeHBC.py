...
def load_training_slices(npz_path="", label_dir="", scales=[3, 6, 12], directions=4):
    """
    Load training slices (STHBC/HER2ST dataset) and construct spatial graph structures.
    """
...
def load_test_dataset(npz_path="", label_path="", scales=[3, 6, 12], directions=4):
    """
    Load the target test dataset (XeHBC) as a single large entity.
    """
...
def train_model(model, train_loader, val_loader, device, epochs=40, lr=1e-4, dist_type="beta"):
    ...
    for epoch in range(epochs):
        ...
        # Save Best Model (selecting based on AUC)
    ...
    # Load best model
    checkpoint = torch.load("temp_best_XeHBC.pth")
...
def evaluate_test_set(model, seed, test_loader, device, val_dist_type, val_dist_params, val_q, best_gamma):
    ...
    # Save Results
    base_dir = "../predict_files/my_model/XeHBC/From_HER2ST"
...
def run_experiment(seed, batch_size, epochs, scales, directions, cl_weight=0.1, dist_type="beta"):
    ...
    train_npz = "../dataset/train_dataset_XeniumHBC.npz"
    test_npz = "../dataset/test_dataset_XeniumHBC.npz"
    train_label_dir = "../data/HB/her2st/label3"
    test_label_path = "../data/HB/Xenium_HBC/spot_labels.txt"
...
def main():
    ...
    print(f"\n{'=' * 60}")
    print(f"{'Spatial Transcriptomics Multi-modal Model Training (XeHBC)':^60}")
    print(f"{'=' * 60}\n")
